...
import { searchDocuments } from '@/utils/ragSearch';
...
export function RAGPage() {
  ...
  const handleSearch = async () => {
    ...
    const matched = searchDocuments(query, docs).slice(0, topK);

    let answer: string;
    if (matched.length === 0) {
      answer = `## 搜索结果\n\n未找到与 **"${query}"** 相关的内容。\n\n知识库共 ${docs.length} 个文档：\n${docs.map((d: any) => `- **${d.filename}** (${d.chunks?.length || d.chunks_count} 块)`).join('\n')}\n\n*使用了停用词过滤+TF-IDF加权算法，过滤了常见的无意义词汇*`;
    } else {
      const topList = matched.map((m, i) => `${i + 1}. **${m.document_name}** [块${m.chunk_id.split('_').pop()}] — ${Math.round(m.score * 100)}%`).join('\n');
      const bestChunk = matched[0]?.fullChunk || matched[0]?.content || '';
      answer = `## RAG 检索结果\n\n查询 **"${query}"** 匹配到 ${matched.length} 个内容块：\n\n${topList}\n\n### 最佳匹配片段\n> ${bestChunk.substring(0, 500)}\n\n---\n*停用词过滤 + TF-IDF加权 | ${docs.length}文档 ${docs.reduce((s: number, d: any) => s + (d.chunks?.length || d.chunks_count || 0), 0)}块*`;
    }
  ...
  const handleUpload = async (file: File) => {
    // Get file content: use backend for PDF/DOCX, FileReader for text
    const ext = file.name.split('.').pop()?.toLowerCase() || '';
    let text = '';
    if (ext === 'pdf' || ext === 'docx' || ext === 'doc') {
      const form = new FormData(); form.append('file', file);
      try {
        const res = await fetch('/api/v1/rag/parse', { method: 'POST', body: form });
        if (res.ok) { const data = await res.json(); text = data.text || ''; }
        else { message.error('解析失败: ' + (await res.text()).substring(0,100)); return false; }
      } catch { message.error('后端解析服务不可用'); return false; }
    } else {
    ...
    if (!text.trim()) {
      message.warning(`${file.name} 无法读取内容，请确认文件格式正确`);
      return false;
    }

    // Real content chunking: 500 chars per chunk with 150 char overlap
    const chunkSize = 500;
    const overlap = 150;
    const chunks: string[] = [];
    let start = 0;
    ...
    saveDocs(updated);
    message.success(`${file.name} 上传成功，已分 ${chunks.length} 块（每块${chunkSize}字符，${overlap}字符重叠）`);
    return false;
  };
