...
async def _init_rag_pipeline(app: FastAPI):
    ...
    #   1. bge-m3 (1024d multilingual) — strongest, used when its weights exist
    #   2. bge-small-zh-v1.5 (512d Chinese) — strong for Chinese, ~95MB
    #   3. hash embedder — deterministic fallback when no model is available
    embedder = None
    try:
        ...
        # Try bge-m3 first (heavier, multilingual)
        for model_key in ("bge-m3", "bge-small-zh-v1.5"):
            try:
                candidate = registry.get_embedder(model_key)
                if getattr(candidate, "_model", None) is not None:
                    embedder = candidate
                    logger.info("Using %s embedder (dim=%d)", model_key, embedder.dim)
                    break
                else:
                    logger.info("%s weights not available, trying next", model_key)
            except Exception as exc:  # noqa: BLE001
                logger.info("%s failed to load (%s), trying next", model_key, exc)
    except Exception as exc:  # noqa: BLE001
        logger.warning("sentence-transformers unavailable (%s)", exc)

    if embedder is None:
        logger.warning("No embedding model available; using hash embedder")
